...
public class DashboardController : Controller
{
    ...
    public async Task<IActionResult> Index(int? selectedCropId = null, int? selectedPlantId = null)
    {
        ...
        try
        {
            ...
            var activeDevicesResult = await _dataQueryService.GetActiveDevicesCountAsync(selectedCropId, selectedPlantId);
            ...
            var thermalStatsResult = await _dataQueryService.GetThermalStatsForDashboardAsync(duration, selectedCropId, selectedPlantId, null);
            if (thermalStatsResult.IsSuccess && thermalStatsResult.Value != null)
            {
                viewModel.ThermalStatistics = thermalStatsResult.Value;
            }

            var latestAmbientDataResult = await _dataQueryService.GetLatestAmbientDataAsync(selectedCropId, selectedPlantId, null);
            ...
            var ambientDataForChartsResult = await _dataQueryService.GetAmbientDataForDashboardAsync(duration, selectedCropId, selectedPlantId, null);
            ...
            var recentCapturesResult = await _dataQueryService.GetThermalCapturesAsync(new DataQueryFilters { PageSize = 5 });
        ...
        catch (Exception ex)
        {
             ...
             ViewData["ErrorMessage"] = "No se pudieron cargar los datos del dashboard. Por favor, intente de nuevo más tarde.";
        }
